...
class PdfBangKeHangXuatView extends StatelessWidget {
  final String tuNgay;
  final String denNgay;
  final TrinaGridStateManager stateManager;
  PdfBangKeHangXuatView({super.key, required this.tuNgay, required this.denNgay, required this.stateManager});
  ...
  Widget build(BuildContext context) {
    final x = pdfBangKeHangXuat(tN: tuNgay, dN: denNgay, dateNow: _dateNow, stateManager: stateManager);
...
Future<Uint8List> pdfBangKeHangXuat({
  required String tN,
  required String dN,
  ...
  stateManager.rows
      ...
      phieu: row.cells[PhieuXuatString.phieu]!.value.toString(),
      ngay: row.cells[PhieuXuatString.ngay]!.value.toString(),
      maKhach: row.cells[PhieuXuatString.maKhach]!.value.toString(),
      maNX: row.cells[PhieuXuatString.maNX]!.value.toString(),
      stt: 0,
      donGia: double.parse(row.cells[PhieuXuatCTString.donGia]!.value.toString()),
      dvt: row.cells[PhieuXuatCTString.dvt]!.value.toString(),
      maHang: row.cells[HangHoaString.maHH]!.value.toString(),
      soLg: double.parse(row.cells[PhieuXuatCTString.soLg]!.value.toString()),
      tenHang: row.cells[HangHoaString.tenHH]!.value.toString(),
      thanhTien: double.parse(row.cells[PhieuXuatCTString.thanhTien]!.value.toString()),
    ),
  )
      .toList();
  String tong = '0';
      ...
      build: (context) {
        return [
          pw.Center(
            child: pw.Column(
              children: [
                pdfWidget.title('BẢNG KÊ HÀNG XUẤT', fontBold),
...
void excelBangKeHangXuat(TrinaGridStateManager stateManager) async {
  final lstData =
  stateManager.rows
      .map(
        (row) => BangKeHangNXModel(
  ...
  final filePath = await getSaveLocation(
    suggestedName: "bangkehangxuat.xlsx",
    acceptedTypeGroups: [
