...
class PdfBangKePhieuThuView extends StatelessWidget {
  final String tuNgay;
  final String denNgay;
  final TrinaGridStateManager stateManager;

  PdfBangKePhieuThuView({super.key, required this.tuNgay, required this.denNgay, required this.stateManager});
  ...
  Widget build(BuildContext context) {
    final x = pdfBangKePhieuThu(tN: tuNgay, dN: denNgay, dateNow: _dateNow, stateManager: stateManager);
    return Scaffold(
      headers: [
        AppBar(
...
Future<Uint8List> pdfBangKePhieuThu({
  required String tN,
  required String dN,
  ...
  final lstData =
      stateManager.rows
          .map(
            (row) => PhieuThuModel(
              phieu: row.cells[PhieuThuString.phieu]!.value.toString(),
              ngay: row.cells[PhieuThuString.ngay]!.value.toString(),
              maKhach: row.cells[PhieuThuString.maKhach]!.value.toString(),
              tenKhach: row.cells[PhieuThuString.tenKhach]!.value.toString(),
              pttt: row.cells[PhieuThuString.pttt]!.value.toString(),
              soTien: double.parse(row.cells[PhieuThuString.soTien]!.value.toString()),
              noiDung: row.cells[PhieuThuString.noiDung]!.value,
              khoa: false,
              soCT: '',
              tkNo: '',
              tkCo: '',
      ...
      build: (context) {
        return [
          pw.Center(
            child: pw.Column(
              children: [
                pdfWidget.title('BẢNG KÊ PHIẾU THU', fontBold),
          ...
          pdfWidget.table(
            ...
            headers: ['STT', 'Ngày', 'Phiếu', 'Mã KH', 'Tên khách hàng', 'PTTT', 'Số tiền', 'Nội dung'],
...
void excelBangKePhieuThu(TrinaGridStateManager stateManager) async {
  final lstData =
  stateManager.rows
      .map(
        (row) => PhieuThuModel(
  ...
  final filePath = await getSaveLocation(
    suggestedName: "bangkephieuthu.xlsx",
    acceptedTypeGroups: [
