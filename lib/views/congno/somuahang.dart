...
class _SoMuaHangViewState extends ConsumerState<SoMuaHangView> {
    ...
    if(_selectKhach!=null){
      ...
      if(numDauKy!=0){
        final ton = await sqlRepository.getCustomData(
          "SELECT SUM(No)-SUM(Co) as No FROM ${ViewName.vbcSoMuaHang} WHERE Ngay < '${Helper.dateFormatYMD(tuNgay)}' AND ${DauKyKhachHangString.maKhach} = '${_selectKhach?.maKhach}'",
        );
        final tonHienTai = await sqlRepository.getCustomData(
          "SELECT SUM(No)-SUM(Co) as No FROM ${ViewName.vbcSoMuaHang} WHERE Ngay BETWEEN '${Helper.dateFormatYMD(tuNgay)}' AND '${Helper.dateFormatYMD(denNgay)}'AND ${DauKyKhachHangString.maKhach} = '${_selectKhach?.maKhach}'",
        );
  ...
  Widget build(BuildContext context) {
    final wNhaCung = ref.watch(lstNhaCungProvider);
            ...
            PrimaryButton(
              ...
              onPressed: () {
                ref
                    ...
                    .getSoMuaHang(tN: tuNgay, dN: denNgay, maKhach: _selectKhach == null ? '' : _selectKhach!.maKhach);
                getTon();
              },
            ),
          ],
            ...
            SizedBox(
              ...
              child: LabelCombobox(
                ...
                onChanged: (val, o) {
                  ref
                      .read(congNoProvider.notifier)
                      .getSoMuaHang(tN: tuNgay, dN: denNgay, maKhach: val??'');
